...
class Particle(object):
    def __init__(self,x=0,y=0,xp=0,yp=0,x0=0,y0=0,width=0,height=0,w=0) -> None:
        self.x=x
        self.y=y
        #self.s =s 
        self.xp=xp
...
class ParticleFilter(object):
    ...
    def initiate(self,measurement):
        ...
        width = ret[2]
        height = ret[3]
        particle = Particle(x_,y_,x_,y_,x_,y_,width,height)
    ...
    def transition(self,w,h):
        for i in range(self.numParticles):
            self.particles[i] = self.calTransition(self.particles[i],w,h)
    def compute(self,tlwhs,x,y):
        sum = 0.0
        for tlwh in tlwhs:
            ret = np.asarray(tlwh).copy()
            ret[:2] += ret[2:]/2
            u1,u2 = ret[:2]
            n = -0.5*(math.pow((x-u1),2)/math.pow(STD_X,2)+math.pow(y-u2,2)/math.pow(STD_Y,2))
            val =math.exp(n)
            sum += val
        print('***',sum)
        return sum
    def updateweight(self,tlwhs):
        sum = 0.0
        for i in range(self.numParticles):
            self.particles[i].w = self.compute(tlwhs,self.particles[i].x,self.particles[i].y)
            sum += self.particles[i].w
        for i in range(self.numParticles):
            self.particles[i].w = float(self.particles[i].w/sum)
    def resample(self):
        n = self.numParticles
        k = 0
        ...
        for i in range(n):
            ...
            if(k==n):
                break
        while k<n:
            new_particles[k] = self.particles[0]
            k = k+1
