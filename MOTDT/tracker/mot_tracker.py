...
class STrack(BaseTrack):
    ...
    def predict(self,w,h):
        ...
        for p in self.particles:
            x = max(0,p.x-p.width/2)
            y = max(0,p.y-p.height/2)
            tlwh = [x,y,p.width,p.height]
            tlwhs.append(tlwh)
        self.tlwhs = np.asarray(tlwhs,dtype=float).reshape(-1,4)
        #xyah

        #self.mean=self.tlwhs[0]
        tlwh = self.tlwhs[0]
        x,y = tlwh[:2]+tlwh[2:]/2
        a = tlwh[2]/tlwh[3]
        h = tlwh[3]
        self.mean = np.asarray([x,y,a,h],dtype=float)
        #?
        #if self.tracker:
            #self.tracker.update_roi(self.tlwh)

    def self_tracking(self, image):
        tlwhs = self.tracker.predict(image) if self.tracker else self.tlwhs
        return tlwhs
    ...
    def activate(self,particle_filter,frame_id,image):
        ...
        self.mean,self.particles =self.particle_filter.initiate(self._tlwh)
        #print('self.tlwh',self._tlwh)
